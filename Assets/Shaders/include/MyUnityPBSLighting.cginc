...
inline void SampleAmbientOcclusion(float2 normalizedScreenSpaceUV)
{
    float2 uv = UnityStereoTransformScreenSpaceTex(normalizedScreenSpaceUV);
    return UNITY_SAMPLE_TEX2D(_ScreenSpaceOcclusionTexture, normalizedScreenSpaceUV)
}
    ...
    inout UnityGI gi)
{
    ...
    data.light = data.light * aoFactor.indirectAmbientOcclusion;
    gi = UnityGlobalIllumination(data,s.Occlusion,s.Normal);
#else
